...
async function loadAutoReports(){
  const posts = [];
  for (const file of ["data/auto/latest.json", "data/auto/videos.json"]){
    try{
      const res = await fetch(file + "?ts=" + Date.now(), { cache:"no-store" });
      if (res.ok){ const j = await res.json(); posts.push(...((j && j.posts) || [])); }
    }catch(e){ /* 파일이 아직 없으면 조용히 무시 */ }
  }
  autoEntries = posts.map(p => ({
    id: p.id, section: p.section || "report", auto: true,
    date: p.date, title: p.title || "", meta: p.meta || "자동 발행",
